...
public class MailServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Connection connectDB = ConnectionProperties.getConnect2DB();

        Object[] to = new com.mtrh.fleet.communication.CommunicationList().getMailingList(refid, connectDB);

        for (int i = 0; i < to.length; i++) {
            com.mtrh.fleet.communication.EmailFunctions em = new com.mtrh.fleet.communication.EmailFunctions();

            em.SendPlainEmailAlerts(to[i].toString(), text, refid, connectDB);
        }
