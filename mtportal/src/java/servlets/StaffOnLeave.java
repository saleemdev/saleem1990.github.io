...
public class StaffOnLeave implements java.lang.Runnable {
    ...
    public void RequestPDF(java.sql.Connection connDb) {
        ...
        connectDB = connDb;
        transaction = "STAFF ON LEAVE REPORT";
        //   leaveID = rqid;
...
//                        docPdf.add(table2);
                        ...
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        phrase = new Phrase("NUMBER OF STAFF ON LEAVE: ", pFontHeader1);
                        table.addCell(phrase);
                        ...
                        table.getDefaultCell().setColspan(5);
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        phrase = new Phrase("DEPARTMENTAL SUMMARY", pFontHeader);
                        table.addCell(phrase);
                        ...
                        for (int i = 0; i < depts.length; i++) {
                            Object dept = depts[i];
                            int numberOfStaffs = getNumberOfStaffOnLeaveInDept(connectDB, dept);

                            table.getDefaultCell().setColspan(3);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase(dept.toString(), pFontHeader1);
                            table.addCell(phrase);

                            table.getDefaultCell().setColspan(2);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase(String.valueOf(numberOfStaffs), pFontHeader1);
                            table.addCell(phrase);
                        ...
                        table.getDefaultCell().setColspan(1);
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        phrase = new Phrase("LEAVE START", pFontHeader);
                        table.addCell(phrase);

                        table.getDefaultCell().setColspan(1);
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        phrase = new Phrase("TO RESUME", pFontHeader);
                        table.addCell(phrase);
...
//for(int i=0;i<depts.length;i)
                        try {
                            for (int j = 0; j < depts.length; j++) {
                                String department = depts[j].toString();
                                table.getDefaultCell().setColspan(5);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("DEPARTMENT: " + department, pFontHeader);
                                table.addCell(phrase);

                                int i = 1;
                                //  for (int i = 0; i < refsonleave.length; i++) {//approved is true

                                //     PreparedStatement pstV1 = connectDB.prepareStatement("select * from hr.leave_application where refno=? ");
                                PreparedStatement pstV1 = connectDB.prepareStatement("select * from hr.leave_application WHERE approved is true AND leavestart::date <= current_date  AND leavestart+daysapproved > current_date  AND pfno IN (SELECT staffid FROM secure_password WHERE upper(department) = UPPER(?))");
                                //    pstV1.setObject(1, refsonleave[i]);
                                pstV1.setObject(1, department);

                                ResultSet rsetV1 = pstV1.executeQuery();

                                System.err.println("select * from hr.leave_application WHERE pfno IN (SELECT refno FROM secure_password WHERE upper(department) = UPPER(?))");
                                ...
                                while (rsetV1.next()) {
                                    ...
                                    Integer days = Integer.valueOf(rsetV1.getString("daysapproved"));

                                    String leavend = LeaveFactory.getLeaveEnd(connectDB, leavestart, days, leavetype);

                                    String resumption = LeaveFactory.getResumptionDate(connectDB, leavend);

                                    table.getDefaultCell().setColspan(2);
                                    table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    phrase = new Phrase(i + ". " + fullname + "/" + staffid, pfontData);
                                    table.addCell(phrase);

                                    table.getDefaultCell().setColspan(1);
                                    table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    phrase = new Phrase(leavetype + " [ " + days + " days]", pfontData);
                                    table.addCell(phrase);

                                    table.getDefaultCell().setColspan(1);
                                    table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    phrase = new Phrase(leavestart, pfontData);
                                    table.addCell(phrase);

                                    table.getDefaultCell().setColspan(1);
                                    table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                    phrase = new Phrase(resumption, pfontData);
                                    table.addCell(phrase);
                                    i += 1;
                                    //   }
                                }
...
//scale the image
    ...
    public static Object[] getDeptsWithPeepsOnLeave(java.sql.Connection conn) {
        ...
        String sql = "select distinct department from secure_password where staffid in (select pfno FROM hr.leave_application WHERE approved is true and leavestart+daysapproved > current_date AND leavestart::date <= current_date )";
        ;
        try {
            PreparedStatement pst = conn.prepareStatement(sql);
            ResultSet rset = pst.executeQuery();
    ...
    public static int getNumberOfStaffOnLeaveInDept(Connection conn, Object dept) {
        int numberOfStaff = 0;
        String sql = "select count(distinct pfno) from hr.leave_application where pfno in (select distinct STAFFID from secure_password where upper(department) ilike upper(?)) and approved is true and leavestart+daysapproved> current_date AND leavestart::date <= current_date";
        ;
        try {
            PreparedStatement pst = conn.prepareStatement(sql);
    ...
    public static Object[] getStaffRefsOnLeave(java.sql.Connection conn) {
        ...
        String sql = "SELECT refno,leavestart,daysapproved, leavetype FROM hr.leave_application WHERE approved is true";
        ;
        try {
            PreparedStatement pst = conn.prepareStatement(sql);
            ResultSet rset = pst.executeQuery();
            while (rset.next()) {
                ...
                String date = rset.getObject(2).toString();
                String lvtype = rset.getObject("leavetype").toString();
                int days = Integer.valueOf(rset.getString(3));
                String leavend = LeaveFactory.getLeaveEnd(conn, date, days, lvtype);

                if (beyond2day(conn, leavend)) {
                    System.err.println("added" + refid);
