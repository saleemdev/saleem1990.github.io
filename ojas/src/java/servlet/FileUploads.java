...
public class FileUploads extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session != null) {
            ...
            try {
                pst = conn.prepareStatement("INSERT INTO vacancies.applicantdocs(\n"
                        + "            applicantid, jobref, doc, description, fileref,remarks)\n"
                        + "    VALUES (?, ?, ?, ?, ?,?);");
                pst.setObject(1, applicantref);
                pst.setObject(2, jobref);
                pst.setBytes(3, bytes);
                pst.setObject(4, desc);
                pst.setObject(5, pkref);
                pst.setObject(6, comments);

                pst.executeUpdate();
                
                
                
                if(desc.equalsIgnoreCase("Passport Size Photo")){
                    pst = conn.prepareStatement("UPDATE vacancies.applicant_bio SET passport = ? WHERE idno = ?;");
                    pst.setObject(1, bytes);
