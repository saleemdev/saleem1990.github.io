 ...
 * @author owner
 */
@WebServlet(name = "Manifest", urlPatterns = {"/Manifest"})
public class Manifest extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Manifest</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Manifest at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session != null) {
            ...
            try {
                PreparedStatement pst = conn.prepareStatement("INSERT INTO fleet.manifest(\n"
                        + "            refid, fullname, idno, contactinfo)\n"
                        + "    VALUES (?, ?, ?,?);");
                System.err.println(fullname+"is where we are");
