...
public class UserData extends HttpServlet {
    ...
    private String getAuthorizingOfficer(java.sql.Connection connectDB) {
        ...
        String sql = "SELECT DISTINCT fullname FROM secure_password WHERE designation ILIKE '%MANAGER%' OR designation ILIKE '%EXECUTIVE%'";
        try {
    ...
    private String getDrivers(java.sql.Connection connectDB) {
        ...
        String sql = "SELECT DISTINCT fullname FROM secure_password where department ILIKE '%transport%'";
        try {
            PreparedStatement pst = connectDB.prepareStatement(sql);
            //   pst.setObject(1, fleetno);
            //  pst.setObject(2, fleetno);
