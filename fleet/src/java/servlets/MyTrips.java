 ...
 * @author owner
 */
@WebServlet(name = "MyTrips", urlPatterns = {"/MyTrips"})
public class MyTrips extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet MyTrips</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet MyTrips at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    private String getMyTrips(Connection connectDB, String driver) {
        ArrayList<HashMap<String, String>> parentList
                = new ArrayList<HashMap<String, String>>();//Parent List

        // Vector v = new Vector();
        String[] columns = new String[]{"rqid", "rqby", "destination", "status", "action"};

        String sql = "SELECT rqid, requestedby, destination, case when journey_start is null then 'not started' else 'enroute' end as status,\n"
                + "case when journey_start is null then 'start trip' else 'end trip' end as action FROM fleet.transportrequestmemo \n"
                + " WHERE trim(driver) ilike ? and approved = true";
        try {
