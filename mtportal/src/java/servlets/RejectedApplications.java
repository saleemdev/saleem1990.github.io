...
public class RejectedApplications implements java.lang.Runnable {
    ...
    public void RequestPDF(java.sql.Connection connDb) {
        ...
        connectDB = connDb;
        transaction = "REJECTED LEAVE APPLICATIONS";
        //   leaveID = rqid;
...
//                        docPdf.add(table2);
                        ...
                        Object[] cancelledrefs = getStaffRefsOnLeave(connectDB);
                        ////
                        ...
                        table.getDefaultCell().setBackgroundColor(java.awt.Color.LIGHT_GRAY);
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        System.err.println("Length:" + cancelledrefs.length);
                        ...
                        try {
                            for (int i = 0; i < cancelledrefs.length; i++) {
                                ...
                                while (rsetV1.next()) {
                                    ...
                                    Object[] messages = getmessagesByrefid(cancelledrefs[i], connectDB);
                                    ...
                                    for (int l = 0; l < messages.length; l++) {
                                        if (l > 0) {
                                            ...
                                            table.getDefaultCell().setColspan(3);
                                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                            phrase = new Phrase(messages[l].toString(), pfontData);
                                            table.addCell(phrase);
...
//            PDFRenderer.renderPDF(tempFile);
    ...
    public static Object[] getmessagesByrefid(Object refid, Connection conn) {
        //select  time::timestamp(0)||': '||message from sms where "messageId" ='MTRH/HR/L/B0842' order by time asc

        Object[] msgs = new Object[]{};
        Vector messagesVector = new Vector();

        String sql = "select  time::timestamp(0)||': '||message from sms where \"messageId\" ='" + refid + "' order by time asc";
        ;
        try {
...
//scale the image
    ...
    public static Object[] getDeptsWithPeepsOnLeave(java.sql.Connection conn) {
        ...
        String sql = "select distinct department from secure_password where staffid in (select pfno FROM hr.leave_application WHERE cancelled is true) ";
        ;
        try {
            PreparedStatement pst = conn.prepareStatement(sql);
            ResultSet rset = pst.executeQuery();
    ...
    public static Object[] getStaffRefsOnLeave(java.sql.Connection conn) {
        ...
        String sql = "SELECT distinct refno FROM HR.leave_application where cancelled is true";
        ;
        try {
            PreparedStatement pst = conn.prepareStatement(sql);
            ResultSet rset = pst.executeQuery();
