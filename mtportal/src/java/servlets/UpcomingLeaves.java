...
public class UpcomingLeaves implements java.lang.Runnable {
    ...
    public void RequestPDF(java.sql.Connection connDb, String leavemonth) {
        ...
        connectDB = connDb;
        transaction = "UPCOMING LEAVES (" + leavemonth + ") - LEAVE YR " + LeaveFactory.getCurrentFY(connDb);
        //   leaveID = rqid;
...
//                        docPdf.add(table2);
                        ...
                        Object[] staffsUpcoming = getStaffsWithUPComingLeave(connectDB, month);
                        ////
                        ...
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        phrase = new Phrase("UPCOMING LEAVES : ", pFontHeader1);
                        table.addCell(phrase);

                        table.getDefaultCell().setColspan(2);
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        phrase = new Phrase("" + staffsUpcoming.length, pFontHeader1);
                        table.addCell(phrase);

                        table.getDefaultCell().setBackgroundColor(java.awt.Color.LIGHT_GRAY);
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);

                        Object[] depts = getDeptsWithUPcomingLeaves(connectDB);

                        table.getDefaultCell().setColspan(5);
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        phrase = new Phrase("DEPARTMENTAL STATS : ", pFontHeader);
                        table.addCell(phrase);
                        ...
                        for (int i = 0; i < depts.length; i++) {
                            Object dept = depts[i];
                            int numberOfStaffs = getNumberOfStaffUpcoming(connectDB, dept);
                        ...
                        table.getDefaultCell().setBackgroundColor(java.awt.Color.LIGHT_GRAY);
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        System.err.println("Length:" + staffsUpcoming.length);
                        ...
                        try {
                            for (int i = 0; i < staffsUpcoming.length; i++) {

                                PreparedStatement pstV1 = connectDB.prepareStatement("select * from hr.leaverota where staffid=?");
                                pstV1.setObject(1, staffsUpcoming[i]);

                                ResultSet rsetV1 = pstV1.executeQuery();

                                while (rsetV1.next()) {

                                    String staffid = rsetV1.getObject("staffid").toString();
                                    String fullname = UserFullName(staffid, connectDB);
                                    String leavetype = rsetV1.getString("leavetype");
                                    Object covering = rsetV1.getObject("coveringofficer");

                                    if (!fullname.contains("None")) {
                                        ...
                                        table.getDefaultCell().setColspan(1);
                                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                        phrase = new Phrase(covering.toString(), pfontData);
                                        table.addCell(phrase);
                                    }
                                }
                            }
...
//scale the image
    ...
    public static Object[] getDeptsWithUPcomingLeaves(java.sql.Connection conn) {
        Object[] depts = new Object[]{};

        Vector deptsInQuestion = new Vector();

        String sql = "select distinct department from secure_password where staffid in (select staffid FROM hr.leaverota WHERE days_plus>0 and upper(month) ilike  '" + month.toUpperCase() + "')";
        ;
        try {
            PreparedStatement pst = conn.prepareStatement(sql);
            ResultSet rset = pst.executeQuery();
    ...
    public static int getNumberOfStaffUpcoming(Connection conn, Object dept) {
        int numberOfStaff = 0;
        String sql = "select count(distinct staffid) from hr.leaverota where STAFFID in (select distinct STAFFID from secure_password where upper(department) ilike upper('" + dept + "'))\n"
                + "and upper(month) ilike '" + month.toUpperCase() + "'";
        ;
        try {
            PreparedStatement pst = conn.prepareStatement(sql);
    ...
    public static Object[] getStaffsWithUPComingLeave(java.sql.Connection conn, String month) {
        Object[] staffsonleave = new Object[]{};
        Vector staffsOnlv = new Vector();

        String sql = "SELECT distinct STAFFID FROM hr.leaverota WHERE days_plus>0 and upper(month) ilike  '" + month.toUpperCase() + "'";
        ;
        try {
            PreparedStatement pst = conn.prepareStatement(sql);
            ResultSet rset = pst.executeQuery();
