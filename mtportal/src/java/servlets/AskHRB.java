 ...
 * @author owner
 */
@WebServlet(name = "AskHRB", urlPatterns = {"/AskHRB"})
public class AskHRB extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AskHRB</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AskHRB at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session != null) {
            ...
            result = getAllMessages(conn);//.replace("[", "").replace("]", "");
    ...
    private String getAllMessages(java.sql.Connection connectDB) {
        ArrayList<HashMap<String, String>> parentList
                = new ArrayList<HashMap<String, String>>();//Parent List

        String[] columns = new String[]{"refno", "uname", "tstamp","read","stat"};
        
        //refno uname timestamp  read  stat

        String sql = "select distinct refno, username, \n"
                ...
                + "FROM hr.notifications where status='S' order by timestamp_ desc;";
    ...
    private String getJustMyMessages(java.sql.Connection connectDB, String staffid) {
        ArrayList<HashMap<String, String>> parentList
                = new ArrayList<HashMap<String, String>>();//Parent List
        ...
        String sql = "select distinct refno, message, \n"
                + "timestamp_::timestamp(0), case when refno in (select refno from hr.notifications where status ='R') then 'REPLIED' else 'NOT REPLIED' end as stat,\n"
                + "CASE WHEN refno in  (select refid from public.workorder) then 'RESOLVED' else 'NOT RESOLVED' END AS stat2\n"
                + "FROM hr.notifications where status='S' AND refno ilike '%"+staffid+"' order by timestamp_ desc;";
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       // processRequest(request, response);
        ...
        if (session != null) {
            ...
            // String leaveyr = getCurrentFY(conn);
            result = getJustMyMessages(conn, staffid);//.replace("[", "").replace("]", "");
