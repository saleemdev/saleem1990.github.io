...
import com.mtrh.fleet.sys.CurrencyFormatter;
...
public class AllMaintenanceTickets implements java.lang.Runnable {
    ...
    public void RequestPDF(java.sql.Connection connDb, String rqid, String reqid) {
        ...
        if (mtcerefno.startsWith("MTRH/MTR/")) {
            transaction = "MAINTENANCE TICKET";
        } else {
            transaction = "FUEL TICKET";
        }
        generatePdf();
    }
...
//                        docPdf.add(table2);
                        ...
                        Object[] vdet = vdao.vehicleDetails(connectDB, vehicle);

                        System.err.println(vdet.length);

                        table.getDefaultCell().setBackgroundColor(java.awt.Color.white);
                        //  table.getDefaultCell().setBorder(Rectangle.BOX);
                        table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                        table.getDefaultCell().setColspan(3);
                        phrase = new Phrase("Vehicle Registration No: " + vehicle + "\n\nModel: " + vdet[10] + "\n\nTransmission " + vdet[7] + "/" + vdet[8] + "\n\nMake: " + vdet[2]+"/"+vdet[3], pFontHeader);
                        table.addCell(phrase);
/////
...
///////////////////////////////////////////////////////////////////////
                        ...
                        table.getDefaultCell().setColspan(5);
                        if (mtcerefno.startsWith("MTRH/MTR/")) {
                            phrase = new Phrase("[MAINTENANCE TICKET ID " + mtcerefno + "]");
                        } else {
                            phrase = new Phrase("[FUEL TICKET ID " + mtcerefno + "]");
                        }
                        table.addCell(phrase);
                        ...
                        try {
                            ...
                            pstV1.setObject(1, mtcerefno);

                            PreparedStatement pstV12 = connectDB.prepareStatement("SELECT * FROM fleet.transport_rq_invoice WHERE requestid = ?");
                            pstV12.setObject(1, mtcerefno);

                            PreparedStatement pstV13 = connectDB.prepareStatement("select * from fleet.vehicle_mileage where refno = ?");
                            pstV13.setObject(1, mtcerefno);
                            //
                            ResultSet rsetV1 = pstV1.executeQuery();
                            ResultSet rsetV12 = pstV12.executeQuery();
                            ResultSet rsetV13 = pstV13.executeQuery();

                            String requestitem = "";
                            Double qty = 0.0, amt = 0.0, mileage = 0.0;

                            while (rsetV13.next()) {
                                mileage = Double.valueOf(rsetV13.getObject(2).toString());
                            }
                            while (rsetV1.next()) {
                                ...
                                Boolean completed =rsetV1.getBoolean("completed");
                                String planned =String.valueOf(rsetV1.getObject("planned_date"));
                                ...
                                table.getDefaultCell().setColspan(3);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Authorized officer: " + requestedby + " at " + enteredon, pfontData);
                                table.addCell(phrase);

                                table.getDefaultCell().setColspan(2);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("MILEAGE(km): " + String.valueOf(mileage) + "\n\n", pfontData);
                                table.addCell(phrase);
                                
                                
                                 table.getDefaultCell().setColspan(3);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Scheduled Date: " + planned, pfontData);
                                table.addCell(phrase);

                                table.getDefaultCell().setColspan(2);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase("Status: " + String.valueOf(stat) + "\n\n", pfontData);
                                table.addCell(phrase);
...
//                                table.addCell(phrase);
                            ...
                            table.getDefaultCell().setColspan(2);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase("Line Item", pfontData);
                            table.addCell(phrase);

                            table.getDefaultCell().setColspan(1);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase("Quantity", pfontData);
                            table.addCell(phrase);

                            table.getDefaultCell().setColspan(1);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase("Cost", pfontData);
                            table.addCell(phrase);

                            table.getDefaultCell().setColspan(1);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                            phrase = new Phrase("Total", pfontData);
                            table.addCell(phrase);
                            Double tot = 0.00;
                            ...
                            while (rsetV12.next()) {
                                requestitem = rsetV12.getObject(2).toString();
                                qty = Double.valueOf(rsetV12.getObject(4).toString());
                                amt = Double.valueOf(rsetV12.getObject(5).toString());

                                table.getDefaultCell().setColspan(2);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(requestitem, pfontData);
                                table.addCell(phrase);

                                table.getDefaultCell().setColspan(1);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(String.valueOf(amt / qty), pfontData);
                                table.addCell(phrase);

                                table.getDefaultCell().setColspan(1);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(String.valueOf(qty), pfontData);
                                table.addCell(phrase);

                                table.getDefaultCell().setColspan(1);
                                table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_LEFT);
                                phrase = new Phrase(String.valueOf(amt), pfontData);
                                table.addCell(phrase);
                                tot = tot + amt;
                            }
                            ...
                            table.getDefaultCell().setColspan(5);
                            table.getDefaultCell().setHorizontalAlignment(PdfCell.ALIGN_RIGHT);
                            phrase = new Phrase("Total: " + CurrencyFormatter.getFormattedDouble(tot), pfontData);
                            table.addCell(phrase);
